...
public sealed class TiptapConverterTests
{
    ...
    [Fact]
    public void TipTapConverter_GetHtmlWordCount_JavaScriptMethodExists()
    {
        Assert.Null(Record.Exception(() =>
        {
            using TiptapConverter tiptapConverter = new();
            return tiptapConverter.GetHtmlWordCount("");
        }));
    }

    [Theory]
    [InlineData(/* lang=html */ "<p><b>Three</b><span id='test'>words</span>&nbsp;here</p>")]
    public void TipTapConverter_GetHtmlWordCount_Success(string html)
    {
        Assert.Null(Record.Exception(() =>
        {
            using TiptapConverter tiptapConverter = new();
            return tiptapConverter.GetHtmlWordCount(html);
        }));
    }
