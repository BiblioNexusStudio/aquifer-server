...
public sealed class EndpointTests(App _app) : TestBase<App>
{
    ...
    public async Task ValidRequest_ShouldReturnSuccess(Request request)
    {
        ...
        {
            result.Count.Should().Be(request.LanguageCodes.Length);
            foreach (var languageCode in request.LanguageCodes)
            {
                result.Should().ContainSingle(x => x.LanguageCode == languageCode);
            }
        }
        ...
        {
            language.LanguageId.Should().BeGreaterThan(0);
            language.LanguageCode.Should().NotBeNullOrEmpty();

            language.ResourceCounts.Should().NotBeNullOrEmpty();
            foreach (var resourceCount in language.ResourceCounts)
            {
