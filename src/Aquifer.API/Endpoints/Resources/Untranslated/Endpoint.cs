...
public class Endpoint(AquiferDbContext dbContext) : Endpoint<Request, List<Response>>
{
    ...
    {
        var chapterRanges = StartAndEndVerseIds(request);

        var rangesSql = chapterRanges != null && chapterRanges.Any()
            ? string.Join(", ", chapterRanges.Select(range => $"({range.Item1}, {range.Item2})"))
    ...
    private static List<(int, int)>? StartAndEndVerseIds(Request request)
    {
        if (request.BookCode is null)
        {
            return null;
        }

        if (request.Chapters is null || request.Chapters.Length == 0)
        {
            var bookId = BookCodes.IdFromCode(request.BookCode);
            return [(BibleUtilities.LowerBoundOfBook(bookId), BibleUtilities.UpperBoundOfBook(bookId))];
        }
        else
        {
            var bookId = BookCodes.IdFromCode(request.BookCode);
            return request.Chapters
                .Select(c => (BibleUtilities.LowerBoundOfChapter(bookId, c), BibleUtilities.UpperBoundOfChapter(bookId, c))).ToList();
        }
    }
}