...
public class Endpoint(AquiferDbContext dbContext) : Endpoint<Request, List<Response>>
{
    ...
    {
        var englishVerses = await GetVerseResourceCountAsync(Constants.EnglishLanguageId, request.ParentResourceId, ct);
        var englishPassages = await GetPassageResourceCountAsync(Constants.EnglishLanguageId, request.ParentResourceId, ct);
        ...
        foreach (var englishRow in rowsEnglishTotals)
        {
            ...
            responseRows.Add(
                new Response
                {
                    ...
                    Status = ParentResourceStatusHelpers.GetStatus(
                        englishRow.TotalResources,
