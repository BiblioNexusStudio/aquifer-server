...
public sealed class QueueClientFactory(string _connectionString, IAzureClientService _azureClientService)
    : IQueueClientFactory
{
    ...
    {
        var client = _connectionString.StartsWith("http")
            ? new QueueClient(new Uri($"{_connectionString}/{queueName}"), _azureClientService.GetCredential(), s_clientOptions)
            : new QueueClient(_connectionString, queueName, s_clientOptions);
