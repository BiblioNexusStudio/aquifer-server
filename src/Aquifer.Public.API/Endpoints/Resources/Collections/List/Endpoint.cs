...
public sealed class Endpoint(AquiferDbReadOnlyContext _dbContext) : Endpoint<Request, IReadOnlyList<Response>>
{
    ...
    public override async Task HandleAsync(Request req, CancellationToken ct)
    {
        var parentResources = await _dbContext.ParentResources
            .Where(pr => pr.Enabled && (req.ResourceType == ResourceType.None || pr.ResourceType == req.ResourceType))
